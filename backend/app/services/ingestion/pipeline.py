...
def _compute_cancer_gender_gap(db: Session, year: int):
    """Compute HEALTH_CANCER_GENDER_GAP_PCT = (men_rate - women_rate) / men_rate * 100."""
    from sqlalchemy import and_

    men_ind = db.query(Indicator).filter(Indicator.code == "HEALTH_CANCER_RATE_MEN").first()
    women_ind = db.query(Indicator).filter(Indicator.code == "HEALTH_CANCER_RATE_WOMEN").first()
    gap_ind = db.query(Indicator).filter(Indicator.code == "HEALTH_CANCER_GENDER_GAP_PCT").first()
    if not (men_ind and women_ind and gap_ind):
        return
    ...
    batch = []
    for muni_id, men_rate in men_rows.items():
        women_rate = women_rows.get(muni_id)
        if women_rate is not None:
            batch.append({
                "municipality_id": muni_id,
                "indicator_id": gap_ind.id,
                "year": year,
                "value": round((men_rate - women_rate) / men_rate * 100, 2),
                "source_file": "derived",
                "sheet_name": "derived",
            })
